...
# 0.54 0.60 0.06
 ...
 # “\\192.168.210.100\mnt\fs1\openpai\zhangxingyan\Fed_ensemblev2.2\save\Result_dataset(cifar10)_N(20)_E(5)_trainnum(500)_P(2)_lr(0.01)_model(LeNet)_M1_MP1_name(cifar10_alpha_0.1_P_2_M1).json”
    file_name = {
        ...
        "FedEns2_S1": FedEnsPath + f'\Result_dataset({dataset})_N(20)_E(5)_trainnum(500)_P(3)_lr(0.01)_model(LeNet)_M2_MP2_name({dataset}_alpha_{alpha}_P_3_M2_S1).json',
        # "FedEns5_S1": FedEnsPath + f'\Result_dataset({dataset})_N(20)_E(5)_trainnum(500)_P(3)_lr(0.01)_model(LeNet)_M5_MP5_name({dataset}_alpha_{alpha}_P_3_M5_S1).json',
        #
        "FedEns2_S2": FedEnsPath + f'\Result_dataset({dataset})_N(20)_E(5)_trainnum(500)_P(3)_lr(0.01)_model(LeNet)_M2_MP2_name({dataset}_alpha_{alpha}_P_3_M2_S2).json',
        # "FedEns5_S2": FedEnsPath + f'\Result_dataset({dataset})_N(20)_E(5)_trainnum(500)_P(3)_lr(0.01)_model(LeNet)_M5_MP5_name({dataset}_alpha_{alpha}_P_3_M5_S2).json',
        #
        "FedEns2_S3": FedEnsPath + f'\Result_dataset({dataset})_N(20)_E(5)_trainnum(500)_P(3)_lr(0.01)_model(LeNet)_M2_MP2_name({dataset}_alpha_{alpha}_P_3_M2_S3).json',
        # "FedEns5_S3": FedEnsPath + f'\Result_dataset({dataset})_N(20)_E(5)_trainnum(500)_P(3)_lr(0.01)_model(LeNet)_M5_MP5_name({dataset}_alpha_{alpha}_P_3_M5_S3).json',
        #
        "FedEns2_S4": FedEnsPath + f'\Result_dataset({dataset})_N(20)_E(5)_trainnum(500)_P(3)_lr(0.01)_model(LeNet)_M2_MP2_name({dataset}_alpha_{alpha}_P_3_M2_S4).json',
        # "FedEns5_S4": FedEnsPath + f'\Result_dataset({dataset})_N(20)_E(5)_trainnum(500)_P(3)_lr(0.01)_model(LeNet)_M5_MP5_name({dataset}_alpha_{alpha}_P_3_M5_S4).json',
    ...
    for name,file in file_name.items():
        try:
            with open(file) as f:
                ...
                num = len(f[key])
                # for i in range(num):
                plt.plot(f[key][:1000], label=name)
